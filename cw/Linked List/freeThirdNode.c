...
int main()
{
    ...
    // Delete the third node
        ...
        struct Node *temp = head->next->next; // Third node
        head->next->next = temp->next;        // Second node points to fourth node
        free(temp);                           // Free memory of third node

    printf("\nAfter deleting the third node:\n");
    displayList(head);
